    ...
    try {
        ...
        if (companyDetails.logoPath) {
            try {
                ...
                const logoWidth = 500; // Adjust the logo width to fit the page
                const pageWidth = 595.28; // A4 page width in points
                const pageHeight = 841.89; // A4 page height in points
        
                const centerX = (pageWidth - logoWidth) / 2; // Positioning logo in the center horizontally
                const centerY = (pageHeight - logoWidth) / 2; // Positioning logo in the center vertically

                // First, add the blurred image as a background with low opacity
                doc.image(logoBuffer, centerX, centerY, { width: logoWidth, opacity: 0.05 }); // Further reduced opacity for subtle effect
            } catch (error) {
