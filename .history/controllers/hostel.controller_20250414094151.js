...
exports.createHostel = async (req, res) => {
    try {
        ...
        const predefinedRules = [
            {
                title: 'Check-In and Check-Out Timing',
                description: 'Guests must check in between 12:00 PM and 10:00 PM and check out by 11:00 AM. Early or late requests are subject to availability.'
            },
            {
                title: 'Cleanliness',
                description: 'Keep your room and shared spaces clean. Dispose of trash responsibly and avoid leaving personal items unattended.'
            },
            {
                title: 'Quiet Hours',
                description: "Maintain silence in dorms and common areas from 10:00 PM to 7:00 AM to respect other guests' comfort."
            }
        ];
       ...
       // const { roomDetails, rules = [], ...hostelData } = req.body;
        ...
        // Merge provided rules with predefined ones (remove duplicates by title)
        const mergedRules = [
            ...predefinedRules,
            ...rules.filter(rule =>
        ...
        if (!hostelData.title || !hostelData.hostelType || !hostelData.guestType || !hostelData.roomIn) {
            return res.status(400).json({
                message: "Missing required fields: title, hostelType, guestType, or roomIn."
            });
        }
        ...
        res.status(201).json({
            ...
            hostel: {
                ...hostel.toObject(),
                rules: mergedRules.map(({ title, description }) => ({ title, description }))
            }
        });

    } catch (error) {
        // Enhanced error logging
        console.error("❌ Error creating hostel:");
        console.error("📦 Request Body:", JSON.stringify(req.body, null, 2));
        console.error("🧑 Host User ID:", req.body?.host || 'Not provided');
