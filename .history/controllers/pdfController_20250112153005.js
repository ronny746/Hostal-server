...
exports.generateBillPDF = async (req, res) => {
    try {
        ...
        // Load Poppins font
        doc.font('path/to/poppins-Regular.ttf'); // Replace with actual path to your font file
        ...
        // Company Details
        doc.fontSize(18).font('Poppins').text(companyDetails.name, { align: 'center' });
        doc.fontSize(10).font('Poppins').text(companyDetails.address, { align: 'center' });
        doc.text(`Phone: ${companyDetails.phone}`, { align: 'center' });
        currentY += 80;

        // Invoice Title
        doc.fontSize(16).font('Poppins').text('INVOICE', 30, currentY, { align: 'center' });
        currentY += 40;

        // Customer and Invoice Details
        doc.fontSize(10).font('Poppins').text(`Invoice Number: ${invoiceNumber}`, 50, currentY);
        ...
        doc.fontSize(10).font('Poppins').font('Helvetica-Bold').text('Subtotal', 400, currentY, { align: 'center' });
        doc.text(`₹${subtotal.toFixed(2)}`, 500, currentY, { align: 'right' });
        currentY += 20;

        doc.fontSize(10).font('Poppins').text('Tax (18%)', 400, currentY, { align: 'center' });
        doc.text(`₹${tax.toFixed(2)}`, 500, currentY, { align: 'right' });
        currentY += 20;

        doc.fontSize(12).font('Poppins').font('Helvetica-Bold').text('Grand Total', 400, currentY, { align: 'center' });
        doc.text(`₹${grandTotal.toFixed(2)}`, 500, currentY, { align: 'right' });

        // Footer (Thank you message centered)
        currentY += 50;
        doc.fontSize(10).font('Poppins').fillColor('#777777')
            .text('Thank you for your business!', 250, currentY, { align: 'center' });
