...
exports.generateBillPDF = async (req, res) => {
    try {
        ...
        doc.font('fonts/Poppins/Poppins-Bold.ttf');
        ...
        if (companyDetails.logoPath) {
            try {
                ...
                const topY = 40; // 30px from the top
        ...
        // Invoice Title
        doc.fontSize(24).font('Poppins-Bold').text('INVOICE', 30, currentY, { align: 'center' });
        currentY += 40;
        ...
        doc.fontSize(10).font('Poppins-Regular').font('Helvetica-Bold').text('Subtotal', 350, currentY, { align: 'center' });
        doc.text(`₹${subtotal.toFixed(2)}`, 500, currentY, { align: 'right' });
        currentY += 20;

        doc.fontSize(10).font('Poppins-Regular').text('Tax (18%)', 350, currentY, { align: 'center' });
        doc.text(`₹${tax.toFixed(2)}`, 500, currentY, { align: 'right' });
        currentY += 20;

        doc.fontSize(12).font('Poppins-Regular').font('Helvetica-Bold').text('Grand Total', 350, currentY, { align: 'center' });
        doc.text(`₹${grandTotal.toFixed(2)}`, 500, currentY, { align: 'right' });

        // Footer (Thank you message centered)
    
        currentY += 50;
        doc.fontSize(20).font('Poppins-Regular').fillColor('#777777')
